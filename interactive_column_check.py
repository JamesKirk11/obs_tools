...
if args.slit_width == 27:
	window,x_range,y_range = plot_slit(f)
	
else: # Using a 40" slit, which we don't have empirical data for so I'm assuming this is well centred
...
if args.pixel != None:
	x = np.array([[args.pixel,np.mean(y_range)]])
	args.nclicks = 1
...
if args.star_distance != None:
	x_comp = [x[i][0] + args.star_distance*60/px_scale for i in range(args.nclicks)]
...
for i in range(args.nclicks):
	...
	if args.y_collapse:
		...
		if args.star_distance != None:
			spectra_comp = data[:,x_comp[i]-args.pixel_width:x_comp[i]+args.pixel_width]
