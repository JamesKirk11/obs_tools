...
parser.add_argument('-yleft','--y_left',type=int,help='NECESSARY ARGUMENT! Rough y value of spectral line that you want to use for the y centroid, for the left hand trace')

parser.add_argument('-xleft','--x_left',type=int,help='User-defined location of left trace, overwriting default finding algorithm. Useful when both stars are in one half of the chip.')
parser.add_argument('-xright','--x_right',type=int,help='Same as -xleft but for right trace.')
...
parser.add_argument('-ap','--spec_w',type=int,help='Number of pixels in x to perform x profile fit over. Default = 40',default=40)
...
parser.add_argument('-bo','--bkg_off',type=int,help='Offset in x between extraction aperture and background aperture. Default = 30',default=30)
parser.add_argument('-bw','--bkg_w',type=int,help='Number of background pixels in x to extract spectra. Default = 20',default=20)
...
nrows,ncols = np.shape(ref_fits[0].data)
nwindows = len(ref_fits)
ref_fits.close()
...
if nwindows == 1 and args.x_left is None:
...
if args.x_left is not None:
    x_beg_left = args.x_left - 20
    x_end_left = args.x_left + 20

    x_beg_right = args.x_right - 20
    x_end_right = args.x_right + 20
...
for i,f in enumerate(l):
    ...
    if hdr["OBJECT"] == obj_name:
        ...
        try:
            ...
            fwhm_x.append(0.12*2*parms_x_left[0][0]*np.sqrt(2**(1/parms_x_left[0][1])-1)*binning) # 0.12 needed for EFOSC arcsec conversion
            fail_left = False
        except:
            print 'Fit failed for left trace'
            parms_x_left.append(0)
            fwhm_x.append(0)
            fail_left = True
        ...
        fwhm_y.append(0.12*2*parms_y_left[0][0]*np.sqrt(2**(1/parms_y_left[0][1])-1)*binning) # 0.12 needed for EFOSC arcsec conversion
        ...
        try:
            ...
            print 'Trace fit failed for right trace'
            parms_x_right = 0
            fail_right = True
