...
parser.add_argument("-ls","--long_slit",help="If using ACAM's 40' slit, use this option.",action="store_true")
...
parser.add_argument("-tc","--target_coords",help="Use if wanting to manually define target coords",nargs='+',type=str)
parser.add_argument("-cc","--comparison_coords",help="Use if wanting to manually define comparison coords",nargs='+',type=str)
parser.add_argument("-mc","--midpoint_coords",help="Use if wanting to manually define midpoint coords",nargs='+',type=str)
...
if args.instrument == 'ACAM':
	...
	if args.long_slit:
		...
		slit_width = 27
		slit_length = 6.8*60 # need this in arcsec
		
	
if args.target_coords:
	targ_ra,targ_dec = args.target_coords
	comp_ra,comp_dec = args.comparison_coords
	mid_ra,mid_dec = args.midpoint_coords
	pa = args.position_angle

if not args.target_coords:
	Vizier.ROW_LIMIT = -1
	...
	comparisons = v.query_region(SkyCoord(ra=target_dict['_RAJ2000'],dec=target_dict['_DEJ2000'], unit=(u.deg,u.deg),frame='icrs'),radius=search_radius,catalog=[args.catalog]) # Query UCAC4 for comparison objects within search radius
...
if args.target_coords:
	...
	MID_RA = args.midpoint_coords[0]
	MID_DEC = args.midpoint_coords[1]
	
	TARG_RA = args.target_coords[0]
	TARG_DEC = args.target_coords[1]
	
	COMP_RA = args.comparison_coords[0]
	COMP_DEC = args.comparison_coords[1]
